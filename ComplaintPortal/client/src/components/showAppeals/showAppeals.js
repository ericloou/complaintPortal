...
export default function BasicTable() {
  ...
  const [modalAppeal, setModalAppeal] = useState({});
  ...
  const handleDelete = async (id) => {
    try {
      const { data } = await axios.delete("http://localhost:5000/appeals", {
        data: { id },
      });
      console.log(data);
      window.location.reload(false);
                    ...
                    <form onSubmit={handleSubmit}>
                      <Box sx={style}>
                        <div className="complaintModal">
                          ...
                          <label>
                            Ticket Number: {modalAppeal.ticketNumber}
                          </label>
                          <label>Message: {modalAppeal.message} </label>
